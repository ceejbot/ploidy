...
fn test_untagged_union_inlines_variants_with_fields() {
    ...
    assert_matches!(
        ...
        [
            ...
            let dog_fields = dog.fields().map(|f| f.name()).collect_vec();
            let cat_fields = cat.fields().map(|f| f.name()).collect_vec();
            matches!(
                &*dog_fields,
    ...
    let variant_struct = match variant.ty().unwrap() {
