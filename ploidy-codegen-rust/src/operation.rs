...
impl ToTokens for CodegenOperation<'_> {
    fn to_tokens(&self, tokens: &mut TokenStream) {
        ...
        // Header parameters become `&str` (or `Option<&str>`) arguments, set on
        // the request via `.header(name, value)` below.
        let header_params = self.op.headers().collect_vec();
        for param in &header_params {
            let ident = CodegenIdentUsage::Param(
                self.graph
                    .ident(IdentMapping::Header(self.op.id(), param.name())),
            );
            if param.required() {
                params.push(quote! { #ident: &str });
            } else {
                params.push(quote! { #ident: Option<&str> });
            }
        }
        ...
        let request = {
            ...
            let header_setters = header_params.iter().map(|param| {
                let ident = CodegenIdentUsage::Param(
                    self.graph
                        .ident(IdentMapping::Header(self.op.id(), param.name())),
                );
                ...
                if param.required() {
                    quote! {
                        let request = request.header(#name, #ident);
                    }
                } else {
                    quote! {
                        let request = if let Some(#ident) = #ident {
                            request.header(#name, #ident)
                        } else {
                            request
                        };
                    }
...
mod tests {
    ...
    #[test]
    fn test_operation_with_header_parameters() {
        let doc = Document::from_yaml(indoc::indoc! {"
            openapi: 3.0.0
            info:
              title: Test API
            ...
            pub async fn get_thing(
                ...
                let result: Result<_, crate::error::Error> = async move {
                    ...
                    let request = {
                        ...
                        let request = request.header("X-Required", x_required);
                        let request = if let Some(x_optional) = x_optional {
                            request.header("X-Optional", x_optional)
                        } else {
                            request
                        };
                        #[cfg(feature = "trace-context")]
